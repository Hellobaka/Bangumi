...
using me.cqp.luohuaming.Bangumi.Core;
using me.cqp.luohuaming.Bangumi.Core.Domain;
using me.cqp.luohuaming.Bangumi.Sdk.Cqp;
using me.cqp.luohuaming.Bangumi.Sdk.Cqp.Enum;
using me.cqp.luohuaming.Bangumi.Sdk.Cqp.EventArgs;
using me.cqp.luohuaming.Bangumi.Sdk.Cqp.Interface;
using me.cqp.luohuaming.Bangumi.Sdk.Cqp.Expand;
using me.cqp.luohuaming.Bangumi.Sdk.Cqp.Model;
using Unity;
using Unity.Injection;

namespace me.cqp.luohuaming.Bangumi.App.Export
{
	/// <summary>	
	/// 表示酷Q事件导出的类	
	/// </summary>	
	...
	{	
		...
		{	
			return "9,me.cqp.luohuaming.Bangumi";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("me.cqp.luohuaming.Bangumi", 1, 9, "酷Q样例应用 for C#", "1.0.0", 1, "JieGG", "酷Q样例应用(V9应用机制)", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("me.cqp.luohuaming.Bangumi", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("me.cqp.luohuaming.Bangumi", AppData.CQLog);	
